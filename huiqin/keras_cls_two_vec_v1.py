...
# df['item_seq_number'] = df['item_seq_number'].apply(lambda x:np.log1p(x))
...
categorical = ["user_id", "region", "city", "parent_category_name", "category_name",
               ...
               ,"Year","Date of Year","Weekday","Weekd of Year","Day of Month","Quarter"
               ]
...
# vec_file2="multilingual_embeddings.ru"
vec_file2="C:/datas/cc.ru.300.vec"
...
embedding_vector2 = make_glovevec(vec_file2,
                                 max_features, embed_size, word_index)

embedding_matrix_final = np.concatenate([embedding_vector, embedding_vector2], axis = 1)
del embedding_vector, embedding_vector2
gc.collect()
...
model = BidLstm(maxlen, max_features,
                embedding_matrix_final,
                embed_size*2)

# Train Model
print("Train nn...")
file_path='simpleRNN_attention_v10.h5'
...
submission.to_csv("gru_sub_v10.csv", index=False)
...
'''
Epoch 00012: val_loss improved from 0.22208 to 0.22200, saving model to simpleRNN_attention_v10.h5
 - 217s - loss: 0.2147 - mean_squared_error: 0.0461 - val_loss: 0.2220 - val_mean_squared_error: 0.0493
Epoch 13/50

Epoch 00009: val_loss improved from 0.22223 to 0.22184, saving model to simpleRNN_attention_v7.h5
 - 138s - loss: 0.2181 - mean_squared_error: 0.0476 - val_loss: 0.2218 - val_mean_squared_error: 0.0492
Epoch 10/50

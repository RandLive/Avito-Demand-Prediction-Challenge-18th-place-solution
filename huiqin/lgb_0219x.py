...
def feature_Eng_On_Deal_Prob(df, df_train):
    ...
    df2 = df
    tmp = df_train.groupby(["price+"], as_index=False)['deal_probability'].median().rename(
        columns={'deal_probability': 'median_deal_probability_price+'})
    df = pd.merge(df, tmp, how='left', on=["price+"])
    df2['median_deal_probability_price+'] = df['median_deal_probability_price+']
    del tmp;
    gc.collect()

    tmp = df_train.groupby(["item_seq_number+"], as_index=False)['deal_probability'].median().rename(
        columns={'deal_probability': 'median_deal_probability_item_seq_number+'})
    ...
    # 0526
    tmp = df_train.groupby(["param_2"], as_index=False)['deal_probability'].median().rename(
        columns={'deal_probability': 'median_deal_probability_param_2'})
    df = pd.merge(df, tmp, how='left', on=["param_2"])
    df2['median_deal_probability_param_2'] = df['median_deal_probability_param_2']

    # 0527
    tmp = df_train.groupby(["city"], as_index=False)['user_id'].count().rename(
        columns={'user_id': 'count_user_id_city'})
    ...
    #
    # 0528 : 0.233263
    tmp = df_train.groupby(["region"], as_index=False)['user_id'].count().rename(
        columns={'user_id': 'count_user_id_region'})
    df = pd.merge(df, tmp, how='left', on=["region"])
    df2['count_user_id_region'] = df['count_user_id_region']

    tmp = df_train.groupby(["wday"], as_index=False)['user_id'].count().rename(
        columns={'user_id': 'count_user_id_wday'})
    df = pd.merge(df, tmp, how='left', on=["wday"])
    df2['count_user_id_wday'] = df['count_user_id_wday']

    tmp = df_train.groupby(["price+"], as_index=False)['user_id'].count().rename(
        columns={'user_id': 'count_user_id_price+'})
    df = pd.merge(df, tmp, how='left', on=["price+"])
    df2['count_user_id_price+'] = df['count_user_id_price+']

    tmp = df_train.groupby(["param_2"], as_index=False)['user_id'].count().rename(
        columns={'user_id': 'count_user_id_param_2'})
    df = pd.merge(df, tmp, how='left', on=["param_2"])
    df2['count_user_id_param_2'] = df['count_user_id_param_2']

    tmp = df_train.groupby(["image_top_1"], as_index=False)['user_id'].count().rename(
        columns={'user_id': 'count_user_id_image_top_1'})
    df = pd.merge(df, tmp, how='left', on=["image_top_1"])
    df2['count_user_id_image_top_1'] = df['count_user_id_image_top_1']

    tmp = df_train.groupby(["user_type"], as_index=False)['user_id'].count().rename(
        columns={'user_id': 'count_user_id_user_type'})
    df = pd.merge(df, tmp, how='left', on=["user_type"])
    df2['count_user_id_user_type'] = df['count_user_id_user_type']

    tmp = df_train.groupby(["parent_category_name"], as_index=False)['user_id'].count().rename(
        columns={'user_id': 'count_user_id_parent_category_name'})
    ...
    tmp = df_train.groupby(["item_seq_number+"], as_index=False)['user_id'].count().rename(
        columns={'user_id': 'count_user_id_item_seq_number+'})
    df = pd.merge(df, tmp, how='left', on=["item_seq_number+"])
...
list_float = ["avg_days_up_user", "avg_times_up_user", "price", "population",
              "price_vs_income", "text_feature_2_words_vs_unique", "blurinesses", "whitenesses", "dullnesses",
              "image_quality",
...
del ridge_preds, ridge_oof_train, ridge_oof_test;
gc.collect()
print("hstack 2 ...")
...
[100]	train's rmse: 0.223962	valid's rmse: 0.226484


Early stopping, best iteration is:
[551]	train's rmse: 0.125075	valid's rmse: 0.232885

Early stopping, best iteration is:
[510]	train's rmse: 0.130025	valid's rmse: 0.233044

Early stopping, best iteration is:
[447]	train's rmse: 0.14379	valid's rmse: 0.233072

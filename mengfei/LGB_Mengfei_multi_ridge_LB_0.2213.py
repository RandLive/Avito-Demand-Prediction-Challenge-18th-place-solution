...
def feature_engineering(df):
    ...
    def Do_Text_Hash(df):
        ...
        df["text_feature_4"] = df.apply(lambda row: " ".join([str(row["param_2"]), str(row["title"])]),axis=1)   
        ...
        df["text_feature_4"] = df["text_feature_4"].apply(lambda x: text_preprocessing(x))
...
def data_vectorize(df):
    ...
    vectorizer = FeatureUnion([
        ...
        ("text_feature_4", CountVectorizer(
            ngram_range=(1, 2),
            preprocessor=get_col("text_feature"))
         ),
    ...
    df.drop(["text_feature", "text_feature_2", "text_feature_4", "description","title", "title_description"], axis=1, inplace=True)

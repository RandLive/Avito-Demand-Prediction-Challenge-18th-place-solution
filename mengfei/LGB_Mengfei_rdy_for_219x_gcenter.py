...
tmp["to_center_dis"] = tmp["city"].copy()
for index in range(len(tmp["to_center_dis"])):
    tmp["to_center_dis"][index] = get_distance(tmp["latitude"][index], tmp["longitude"][index])
...
#        df["image_top_4"].fillna(-1,inplace=True)
        ...
        df["to_center_dis"].fillna(-1,inplace=True)

...
#del full_df["image"]; gc.collect() # 0.236355
...
xgbval = xgb.DMatrix(X_valid, y_valid)

watchlist = [(xgbtrain, 'train'), (xgbval, 'valid')]
model_xgb = xgb.train(xgb_params, xgbtrain, 2000, watchlist, maximize=False, early_stopping_rounds = 500, verbose_eval=100)

dtest = xgb.DMatrix(test)
xgbpred = model_xgb.predict(dtest, ntree_limit=model_xgb.best_ntree_limit)
xgbsub = pd.DataFrame(xgbpred,columns=["deal_probability"],index=sub_item_id)
xgbsub["deal_probability"].clip(0.0, 1.0, inplace=True) # Between 0 and 1
xgbsub.to_csv("ml_xgb_sub.csv",index=True,header=True)
print("XGB Done.")

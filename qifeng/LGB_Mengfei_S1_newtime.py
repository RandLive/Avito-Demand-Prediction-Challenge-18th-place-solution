...
if debug == False:
    ...
    train_periods = pd.read_csv("input/periods_train.csv", parse_dates=["date_from", "date_to", "activation_date"])
    test_periods = pd.read_csv("input/periods_test.csv", parse_dates=["date_from", "date_to", "activation_date"])
else:
    train_df = pd.read_csv("input/train.csv", parse_dates = ["activation_date"])
    ...
    train_periods = pd.read_csv("input/periods_train.csv",  nrows=1000, parse_dates=["date_from", "date_to", "activation_date"])
    test_periods = pd.read_csv("input/periods_test.csv",  nrows=1000, parse_dates=["date_from", "date_to", "activation_date"])
...
del all_samples, n_user_items
gc.collect()

train_df = train_df.merge(gp, on="user_id", how="left")
test_df = test_df.merge(gp, on="user_id", how="left")

train_df = train_df.merge(all_periods[['item_id', 'days_wait', 'date_to']], on='item_id', how='left')
test_df = test_df.merge(all_periods[['item_id', 'days_wait', 'date_to']], on='item_id', how='left')

agg_cols = list(gp.columns)[1:]

del gp, all_periods; gc.collect()
...
def feature_engineering(df):
    ...
    def Do_Datetime(df):
        ...
        df["endday"] = df["date_to"].dt.weekday
        df["endday"].fillna(-1,inplace=True)
        df["endday"] = df["endday"].astype(np.uint8)
...
#        df["price"].fillna(-1,inplace=True)
    ...
    def Do_Drop(df):
        df.drop(["activation_date", "item_id", 'date_to'], axis=1, inplace=True)

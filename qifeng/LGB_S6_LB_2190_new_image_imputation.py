...
def feature_engineering(df):
    ...
    def Do_impute(df):
        print('Imputating ...')
        df = pd.DataFrame(df)
        tmp_df = df[['image_top_1', 'param_2', 'param_3', 'city', 'region', 'param_1', 'category_name', 'parent_category_name','user_type']]
        tmp_df = tmp_df.replace(0, np.nan)
        tmp_df = pd.DataFrame(data=MICE().complete(tmp_df), columns=tmp_df.columns, index=tmp_df.index)
        df.drop(['image_top_1', 'param_2', 'param_3', 'city', 'region', 'param_1','category_name', 'parent_category_name','user_type'], axis=1, inplace=True)
        df.join(tmp_df)

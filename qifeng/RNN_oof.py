...
mean_train_rmse = 0.
mean_valid_rmse = 0.
val_predict = np.zeros(y_train.shape)
...
for train_idx, valid_idx in kf.split(y_train):
    ...
    X_train1, X_test1 = train1[train_idx], train1[valid_idx]
    print(X_train1.shape, X_test1.shape)
    y_train1, y_test1 = y_train[train_idx], y_train[valid_idx]
    ...
    full_df = pd.concat([X_train_final, X_test_final])
    ...
    print("Predictions done for Fold "+str(kf))
    print(preds.shape)
    ...
    pred_test = modelRNN.predict(test, batch_size=4000)
    pred_test = pred_test.flatten()
    pred_valid = modelRNN.predict(X_test_f, batch_size=4000)
    pred_train = modelRNN.predict(X_train_f, batch_size=4000)
    

    print('validation rmse: ', rmse(y_test1, pred_valid.flatten()))
    print('train rmse: ', rmse(y_train1, pred_train.flatten()))
            
    val_predict[valid_idx] = pred_valid.flatten()
    ...
    mean_train_rmse += rmse(y_train1, pred_train.flatten())
    mean_valid_rmse += rmse(y_test1, pred_valid.flatten())   
...
val_predicts = pd.DataFrame(data=val_predict, columns= y_train)

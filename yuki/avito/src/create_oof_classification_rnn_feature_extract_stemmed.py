...
is_trainable = 0
EMBEDDING_SIZE = 100
EMBEDDING_FILE = "../model/fasttext_model_stemmed.bin"
activation = "sigmoid"
classification_type = "bin"# "high"
...
try:
    ...
    for text in tqdm(list(X) + list(X_test)): # max_length, padding,
        for w in text.split():
            try:
                embedding_vectors[w] = model.get_word_vector(w)#model[w]
                if w in word_idx:
                    continue
                else:
    ...
    df_out = pd.DataFrame(result, columns=["oof_rnn_stemmed_classification_{}_".format(classification_type)])
    to_parquet(df_out, "../features/oof_rnn_stemmed_classification_{}_test.parquet".format(classification_type))

    df_out = pd.DataFrame(oof_valid, columns=["oof_rnn_stemmed_classification_{}_".format(classification_type)])
    to_parquet(df_out, "../features/oof_rnn_stemmed_classification_{}_train.parquet".format(classification_type))

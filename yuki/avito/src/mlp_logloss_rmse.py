...
parser.add_argument('--loss', '-l', default="binary_crossentropy", help='loss function')
...
if is_read_data:
	...
	print("scale data")
	scaler = GaussRankScaler() #GaussRankScaler
	...
	pq.write_table(arrow_table_train, "../tmp/X_train_scaled.parquet")
	pq.write_table(arrow_table_test, "../tmp/X_test_scaled.parquet")
else:
	print("read parquet data...")
	X = pq.read_table("../tmp/X_train_scaled.parquet").to_pandas()
	X_test = pq.read_table("../tmp/X_test_scaled.parquet").to_pandas()
...
for ix_train, ix_valid in kfolds:
	...
	bst_model_path = 'dnn_rmse_{}_{}.h5'.format(arch,loss_function)
...
output_labels = ["oof_stacking_level1_mlp_rmse_{}_{}".format(arch,loss_function)]
...
to_parquet(df_oof_val,"../features/oof_stacking_level1_mlp_rmse_{}_{}_train.parquet".format(arch,loss_function))
to_parquet(df_oof_test, "../features/oof_stacking_level1_mlp_rmse_{}_{}_test.parquet".format(arch,loss_function))

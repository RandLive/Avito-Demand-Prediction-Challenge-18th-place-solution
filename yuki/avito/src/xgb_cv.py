...
try:
    ...
    sub["oof_stacking_level1_xgb_seed42_{}".format(version)] = 0
    ...
    for ix_train, ix_valid in kfolds:
        ...
        sub["oof_stacking_level1_xgb_seed42_{}".format(version)] += bst.predict(d_test) / 5
        y_valid_pred = bst.predict(d_val)
        val_pred[ix_valid] = y_valid_pred
        cv_scores.append(rmse(y_valid_fold, y_valid_pred))

    cv_score = np.mean(cv_scores)
    train_oof = pd.DataFrame(val_pred, columns=["oof_stacking_level1_xgb_seed42_{}".format(version)])
    sub = sub.drop("item_id", axis=1)
    to_parquet(train_oof, "../stacking/oof_stacking_level1_xgb_seed42_{}_train.parquet".format(seed, version))
    to_parquet(sub, "../stacking/oof_stacking_level1_xgb_seed42_{}_test.parquet".format(seed, version))
